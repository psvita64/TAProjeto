...
public class Gestor {
    ...
    public static void MostrarFaturasFuturas(ArrayList<Animal> listaAnimal, ArrayList<Cliente> listaClientes, ArrayList<Veterinario> listaVeterinarios, ArrayList<Intervencao> listaIntervencoes, ArrayList<Data> listaDatas, ArrayList<Recibo> listaRecibos){
        int op, tipo, idAnimal = 0, nif = 0;
        Animal animal = null;
        Cliente cliente = null;
        Intervencao intervencao = null;
        ArrayList<Intervencao> list = new ArrayList<>();
        float preco = 0;

        System.out.println("Que tipo de intervenção deseja listar as faturas? (1- Consulta, 2- Cirurgia, 3- Vacinação)");
        do {
            while (!sc.hasNextInt()) {
            ...
            switch (tipo){
                case 1:
                    intervencao = new Consulta(null,null,0,false,null);
                    break;
                case 2:
                    intervencao = new Cirurgia(null,null,0,false,null);
                    break;
                case 3:
                    intervencao = new Vacinacao(null,null,0,false,null);
                    break;
                default:
                    System.out.println("\nOpção inválida\n");
            }
        }while(intervencao == null);
        ...
        }while(nif <= 99999999 || nif > 999999999 || cliente == null);
        for(Intervencao i: listaIntervencoes){
            if(i.getClass().equals(intervencao.getClass())) {
                if(i.getData().getDia() <= 23 && i.getData().getMes() <= 5){
        ...
        System.out.println("Faturação: \n " +
                "Tipo de Intervenção: " + intervencao.getClass() + "\n " +
                "Nome Cliente: " + animal.getCliente().getNome() + "\n " +
                "Nome Animal: " + animal.getNome() + "\n " +
                "Soma Total: " + preco + "€");
    }

    public static void MostrarFaturasPassadas(ArrayList<Animal> listaAnimal, ArrayList<Cliente> listaClientes, ArrayList<Veterinario> listaVeterinarios, ArrayList<Intervencao> listaIntervencoes, ArrayList<Data> listaDatas, ArrayList<Recibo> listaRecibos){
        int op, tipo, idAnimal = 0, nif = 0;
        Animal animal = null;
        Cliente cliente = null;
        Intervencao intervencao = null;
    ...
    public static void HorarioExits(ArrayList<Horario> listaHorario, Data data){
        ...
        System.out.println("Não existe horário para a data " + data.getDia() + "/" + data.getMes() + ".");
    }
    ...
    public static Intervencao addIntervencao(ArrayList<Intervencao> listaIntervencao, ArrayList<Cliente> listaClientes, ArrayList<Veterinario> listaVeterinarios, ArrayList<Animal> listaAnimais, ArrayList<Data> listaData, ArrayList<Recibo> listaRecibos){
        ...
        do {
            ...
            System.out.println("0 - Voltar\n");
                ...
                sc.nextLine();
                if (tipo== 1)
                    intervencao = "Consulta";
                else if (tipo == 2)
                    intervencao = "Cirurgia";
                else if (tipo == 3)
                    intervencao = "Vacinacao";
                        ...
                        for (Cliente c : listaClientes) {
                            if (c.getNif() == nif) {
                                ...
                                do {
                                    ...
                                    if (animal == null) {
                                        System.out.println("\nID inválido: Animal não encontrado.\n");
                                    }
                                } while (animal == null);
                                break;
                            }
            ...
            do {
                ...
                do {
                    ...
                    date = LocalDate.of(2024,mes,dia);
                    if(isBeforeToday(date)){
                        System.out.println("Data inválida: Insira uma data depois de hoje.");
                    }
                }while(isBeforeToday(date));
                sc.nextLine();
                ...
                do {
                    ...
                    if(hora <= 8 || hora > 18 || hora%0.5 != 0  ){
                        System.out.println("Insira uma Hora Válida.");
                    }
                } while(hora <= 8 || hora > 18 || hora%0.5 != 0 );
                sc.nextLine();

                d = new Data(dia,mes);
                if(!veterinario.TemHorario(d)){
            ...
            }while (dia <= 0 || dia > 31 || mes <= 0 || mes > 12 || hora < 8 || hora > 18 || hora%0.5 != 0 || !horario.isHoraLivre(hora, d));
            ...
            do {
                ...
                if(op != 1 && op != 2){
                    ...
                    switch (tipo) {
                        case 1:
                            Consulta consulta = new Consulta(veterinario, animal, preco, deslocacao, d);
                            ...
                            return null;
                        case 2:
                            Cirurgia cirurgia = new Cirurgia(veterinario, animal, preco, deslocacao, d);
                            ...
                            return null;
                        case 3:
                            Vacinacao vacinacao = new Vacinacao(veterinario, animal, preco, deslocacao, d);
    ...
    public static Cliente AddCliente( ArrayList<Cliente> clientes){
        ...
        System.out.println("Insira a numero da porta do cliente : ");
        while (true) {
            if (sc.hasNextInt()) {
                numero = sc.nextInt();
                sc.nextLine(); // Limpar o buffer
